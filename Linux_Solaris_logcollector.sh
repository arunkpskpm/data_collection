...
if [ -f "/var/tmp/server_info/server_info.txt" ];then
   cp /var/tmp/server_info/server_info.txt /var/tmp/server_info/server_info-`date +%Y%m%d_%H_%M`.txt
...
printf "\n# fmdump \n$(fmdump 2>/dev/null)\n" >>$LOG
	...
	;;
	*)
printf "\n Unsupported OS - Supported Operating Systems are : Solaris 9(partially),10 and Red Hat EL 5.x, 6.x \n\n" 1>&2
exit 1
	;;
esac
