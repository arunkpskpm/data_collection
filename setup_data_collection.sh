...
if [ -f "/etc/redhat-release" ]
   ...
   then
   echo "#Script placed by UNIX team to collect performance data" >> /var/spool/cron/tabs/root
   echo "* * * * * /var/tmp/srvmon/perf_monitor.sh  >/dev/null 2>&1 " >> /var/spool/cron/tabs/root
fi
if crontab  -l |grep perf_monitor.sh >/dev/null 2>&1; then
