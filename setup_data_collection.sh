...
DATE=`date`
TOPCPU=`top -d 5 -n 1 -b | head -30 |sed 's/%/%%/g'`           #Top process utilization sorted based on CPU usage
TOPMEM=`top -a -d 5 -n 1 -b | head -30 |sed 's/%/%%/g'`        #Top process utilization sorted based on Memory usage
...
/var/tmp/srvmon/top.log /var/tmp/srvmon/swap.log /var/tmp/srvmon/vmstat.log /var/tmp/srvmon/iostat.log /var/tmp/srvmon/pidstat.log /var/tmp/srvmon/sar_cpu.log /var/tmp/srvmon/sar_mem.log
{
    compress
    weekly
    rotate 6
...
#Defining variables
DATE=`date`
PRSTAT=`prstat -s cpu  -can 100 1 1|sed 's/%/%%/g'`		# prstat sorted based on cpu utilization. Enable either of it based on situation.
#PRSTAT=`prstat -s rss  -can 100 1 1|sed 's/%/%%/g'` 		# prstat sorted based on memory utilization. Enable either of it based on situation.
SWAP=`swap -s | awk ' { print $9 $11 } ' | awk -Fk '{print $1 * 100 / ( $1 + $2 ) }'`
VMSTAT=`vmstat 1 5`
IOSTAT=`iostat -xnmpz 2 5 |tail -200|sed 's/%/%%/g'`
LOG_PRSTAT="/var/tmp/srvmon/prstat.log"
LOG_SWAP="/var/tmp/srvmon/swap.log"
LOG_VMSTAT="/var/tmp/srvmon/vmstat.log"
LOG_IOSTAT="/var/tmp/srvmon/iostat.log"
#Writing logs collected
...
#Setting up log rotation 
logadm -w /var/tmp/srvmon/prstat.log -C 6 -p 1w -c -z 0
